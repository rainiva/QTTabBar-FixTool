...
长期稳定恢复建议使用“健康快照”机制：在 QTTabBar 正常显示时先保存一份健康快照；以后如果又退化成只剩菜单条，可优先恢复这份快照，而不是依赖实验性布局重建。
...
# UI 重置辅助（会在倒计时后向前台资源管理器发送 F11 两次）
.\Fix-QTTabBar.ps1 -ResetUi

# 保存当前健康快照（建议在 QTTabBar 正常显示时执行一次）
.\Fix-QTTabBar.ps1 -SaveSnapshot

# 恢复最近一次健康快照
.\Fix-QTTabBar.ps1 -RestoreSnapshot

# 实验性布局重置（会清除损坏的 QTTabBar band 布局；若新布局未生成，会自动回滚）
.\Fix-QTTabBar.ps1 -ResetLayout
```
...
| 1 | 完整修复（阶段一 + 阶段二）**推荐** |
| 2 | 健康检测 |
| 3 | UI 重置辅助（向前台资源管理器发送 F11 两次） |
| 4 | 保存当前健康快照 |
| 5 | 恢复最近一次健康快照 |
| 0 | 退出 |

阶段一、阶段二、查询、模拟运行、快速清理和实验性布局重置仍保留命令行入口，适合排查时按需使用。
...
└── Backup/                   # 注册表备份与 Snapshots 快照（自动生成，已 gitignore）
```
...
- 保存健康快照时会在 `Backup\Snapshots\healthy-时间戳\` 下写入一组可恢复的 `.reg` 文件
